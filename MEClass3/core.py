...
def exec_proc_reg(args):
    ...
    #
    gap_join = args.gtj_inp #500    
    max_size = args.msz_inp #10000
    path_to_output = os.getcwd()
    if not os.path.isdir('enhc_bed'):
        os.makedirs('enhc_bed')    
#    path_to_output = args.pto_inp
    #
    enhc_fofn = args.efn_inp
    ...
    for i in chr_list:
        ...
        df_chrom[chrom] = df_chrom[chrom].drop_duplicates(subset=['thickStart', 'thickEnd'])
        df_chrom[chrom].to_csv(path_to_output+'/enhc_bed/'+chrom+'_all_cell_type.bed', sep='\t', index=False)

        #Condolidate regions
        df_chrom_cons[chrom] = pd.DataFrame(columns = ['chrom', 'thickStart', 'thickEnd', 'length'])
        df_chrom_cons[chrom][['thickStart', 'thickEnd', 'length']] = \
                df_chrom_cons[chrom][['thickStart', 'thickEnd', 'length']].astype(int)
        prev_start = df_chrom[chrom].loc[0, 'thickStart']
        prev_end = df_chrom[chrom].loc[0, 'thickEnd']
        for item in df_chrom[chrom].index:
            if df_chrom[chrom].loc[item, 'thickStart'] <= prev_end + gap_join:
                if df_chrom[chrom].loc[item, 'thickEnd'] > prev_end:
                    prev_end = df_chrom[chrom].loc[item, 'thickEnd']
            if df_chrom[chrom].loc[item, 'thickStart'] > prev_end + gap_join:
                if prev_end-prev_start <= max_size:
                    df_chrom_cons[chrom].loc[item] = [chrom, prev_start, prev_end, prev_end-prev_start]
                prev_start = df_chrom[chrom].loc[item, 'thickStart']
                prev_end = df_chrom[chrom].loc[item, 'thickEnd']
...
def main():
    ...
    sp_proc_reg = subparsers.add_parser('proc_reg', help='proc reg argument')
    sp_proc_reg.add_argument('-gtj', action='store', dest='gtj_inp', type=int, default=500, help='maximum gap to join together')
    sp_proc_reg.add_argument('-msz', action='store', dest='msz_inp', type=int, default=10000, help='maximum size of re')
    sp_proc_reg.add_argument('-efn', action='store', dest='efn_inp', help='fofn enhc prediction')
