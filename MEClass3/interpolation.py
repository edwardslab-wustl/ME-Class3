    ...
    with open(args.logfile, 'w') as log_FH:
        ...
        for sample_pair in pair_list:
            ...
            if anno_type == 'gene_tss':
                ...
                if args.num_proc == 0:
                    out_data, gene_fail_dict = interp_genes_sp(anno_list_postfilter, dict_bed, sample_id, gene_fail_dict, log_FH, args)
                else:
                    out_data, gene_fail_dict = interp_genes_mp(anno_list_postfilter, dict_bed, sample_id, gene_fail_dict, log_FH, args)
                ...
                for status in gene_fail_dict:
                    ...
                    print_to_log(log_FH, str(num_fail) + " genes failed to pass " + status + " filter.\n")
...
def interp_genes_sp(gene_list, dict_bed, sample_id, fail_dict, log_FH, args):
    final_results = []
    #fail_dict = defaultdict(list)
    for gene in gene_list:
        (status, result) = interp_gene(gene, dict_bed, sample_id, args)
        if status == 'pass':
...
def interp_genes_mp(gene_list, dict_bed, sample_id, fail_dict, log_FH, args):
...
def exec_interp_help(parser):
    ...
    parser.add_argument('--num_proc', type=int, default=0, help='number of processes to run')
